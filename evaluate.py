...
#Large testing case with the precedence used by pythons eval
...
#Small example using different precedences
expr="3+4*2+5"
opers={"*":op.mul,"+":op.add}

print(f"mul before add: {expr} = ",evaluate(expr,opers,prs={"+":(2,3),"*":(4,5)}))

print(f"add before mul: {expr} = ",evaluate(expr,opers,prs={"*":(2,3),"+":(4,5)}))

print(f"left before right: {expr} = ",evaluate(expr,opers,prs={"*":(2,3),"+":(2,3)}))
